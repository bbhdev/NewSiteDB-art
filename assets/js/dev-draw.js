...
(function () {
  ...
  const state = {
    ...
    // Character styles (TS4 Slice 3). Site-wide named RELATIVE inline
    // styles, shared across pages. Persisted separately via
    // /dev/draw/charstyles — like typography, outside undo/snapshot.
    // Entries carry ONLY the fields the author set (unset → inherit).
    charStyles: Array.isArray(initial.charStyles) ? initial.charStyles : [],
  ...
  // ════════════════════════════════════════════════════════════════════
  // Character-style panel (TS4 Slice 3). A near-clone of the typography
  // panel above, reusing its .ed-typo-* CSS (identical row structure).
  // KEY DIFFERENCE: char-style fields are OPTIONAL — an entry carries ONLY
  // the fields the author set; unset fields are absent (the key is deleted)
  // so they inherit at render. Mirrors the server's omit-unset validator and
  // deco_charstyle_marks_css emitter. Persisted via /dev/draw/charstyles,
  // separate from the per-page draw save (so outside undo/snapshot history).
  // ════════════════════════════════════════════════════════════════════
  const newCsIds = {};        // ids that still track the name until first save
  const expandedCsIds = {};   // session: which rows show their field editor
  let charStylesDirty = false;

  // weight/italic are <select>s with an explicit "inherit" choice (value '')
  // distinct from a concrete value — that's how the tri-state reaches data.
  const CS_WEIGHTS = [
    { value: '',        label: '— inherit —' },
  ...
  function slugifyCsId(name) {
    const s = String(name || '').toLowerCase()
      .replace(/[^a-z0-9_-]+/g, '-').replace(/-+/g, '-').replace(/^-+|-+$/g, '');
    return s || 'style';
  }
  function uniqueCsId(base, exceptId) {
    const taken = {};
    (state.charStyles || []).forEach(function (c) { if (c.id !== exceptId) taken[c.id] = true; });
    if (!taken[base]) return base;
    let n = 2;
    while (taken[base + '-' + n]) n++;
    return base + '-' + n;
  ...
  // Compact one-line summary of the overrides this style carries.
  function csSpecLine(c) {
    const parts = [];
    if (c.sizeEm != null && c.sizeEm !== '') parts.push(c.sizeEm + 'em');
    if (c.weight === 'bolder' || c.weight === 'lighter') parts.push(c.weight);
    if (c.italic === true) parts.push('italic');
    else if (c.italic === false) parts.push('upright');
    if (c.letterSpacingEm != null && c.letterSpacingEm !== '') parts.push('ls ' + c.letterSpacingEm + 'em');
    return parts.length ? parts.join(' · ') : 'inherit (no overrides)';
  }

  // Live <style> mirroring deco_charstyle_marks_css() — bare .mk-cs-<id>
  // rules (same single-class specificity = the M1/M2 cascade), emitting ONLY
  // set fields. Wins over the server's #ed-charstyle-css on equal specificity
  // (inserted after). Keeps the panel previews WYSIWYG for unsaved edits.
  function rebuildCharstyleClientCss() {
    let el = document.getElementById('ed-charstyle-css-live');
    if (!el) {
      el = document.createElement('style');
      el.id = 'ed-charstyle-css-live';
      document.head.appendChild(el);
    }
    const css = (state.charStyles || []).map(function (c) {
      const id = String(c.id || '').replace(/[^a-z0-9_-]/gi, '');
      if (!id) return '';
      const decls = [];
      if (c.sizeEm != null && c.sizeEm !== '') {
        const em = clampTypoNum(c.sizeEm, 0.1, 10, NaN);
        if (!isNaN(em)) decls.push('font-size: ' + em + 'em');
      }
      if (c.weight === 'bolder' || c.weight === 'lighter') decls.push('font-weight: ' + c.weight);
      if (c.italic === true || c.italic === false) decls.push('font-style: ' + (c.italic ? 'italic' : 'normal'));
      if (c.letterSpacingEm != null && c.letterSpacingEm !== '') {
        const ls = clampTypoNum(c.letterSpacingEm, -2, 2, NaN);
        if (!isNaN(ls)) decls.push('letter-spacing: ' + ls + 'em');
      }
      return decls.length ? ('.mk-cs-' + id + ' { ' + decls.join('; ') + '; }') : '';
    }).filter(Boolean).join('\n');
    el.textContent = css;
  }

  function markCharStylesDirty() {
    charStylesDirty = true;
    const btn = document.getElementById('save-charstyles-btn');
    if (btn) { btn.classList.add('is-dirty'); btn.textContent = 'Save •'; }
  }
  function clearCharStylesDirty() {
    charStylesDirty = false;
    const btn = document.getElementById('save-charstyles-btn');
    if (btn) { btn.classList.remove('is-dirty'); btn.textContent = 'Save'; }
  }

  // A number field whose BLANK value means "unset" (→ inherit). Calls
  // onChange(null) on blank, onChange(number) otherwise. Placeholder shows
  // "inherit" so the affordance is clear.
  function csEmField(label, value, onChange) {
    const wrap = document.createElement('div');
  ...
  function addCharStyle() {
    if (!Array.isArray(state.charStyles)) state.charStyles = [];
    const id = uniqueCsId('style');
    // New style starts with NO overrides — pure inherit until the author
    // sets a field. (Matches the data model: only set fields are stored.)
    state.charStyles.push({ id: id, name: 'New style' });
    newCsIds[id] = true;
    expandedCsIds[id] = true;
    rebuildCharstyleClientCss();
    markCharStylesDirty();
    renderCharStyleList();
    const inp = document.querySelector(
      '#charstyle-list .ed-typo-row[data-cs-id="' + id + '"] .ed-typo-name-input');
    if (inp) { inp.focus(); inp.select(); }
  }

  function deleteCharStyle(c) {
    if (!confirm('Delete character style "' + (c.name || c.id) +
                 '"? Text using it falls back to its base + atomic styles.')) return;
    state.charStyles = (state.charStyles || []).filter(function (x) { return x.id !== c.id; });
    delete newCsIds[c.id];
    rebuildCharstyleClientCss();
    markCharStylesDirty();
    renderCharStyleList();
  }

  function renderCharStyleList() {
    const listEl = document.getElementById('charstyle-list');
    if (!listEl) return;
    listEl.innerHTML = '';
    const styles = Array.isArray(state.charStyles) ? state.charStyles : [];
    if (!styles.length) {
      const empty = document.createElement('li');
      empty.className = 'ed-typo-empty';
      empty.textContent = 'No character styles yet — use “+ Style”.';
      listEl.appendChild(empty);
      return;
    }
    styles.forEach(function (c) {
      const li = document.createElement('li');
      li.className = 'ed-typo-row';
      li.setAttribute('data-cs-id', c.id);
      ...
      toggle.type = 'button';
      toggle.className = 'ed-mini ed-typo-toggle';
      toggle.title = 'Edit this style’s overrides';
      ...
      const idTag = document.createElement('code');
      idTag.className = 'ed-typo-id';
      idTag.textContent = 'cs-' + c.id;
      idTag.title = newCsIds[c.id]
        ? 'Stable id (tracks the name until first saved, then locks)'
        : 'Stable id — does not change on rename (marks reference it)';
      ...
      del.addEventListener('click', function () { deleteCharStyle(c); });

      // Live sample — shows the RELATIVE effect: plain text then the same
      // text wrapped in .mk-cs-<id>, so size/weight/italic deltas are visible
      // against the surrounding base.
      const sample = document.createElement('div');
      sample.className = 'ed-typo-sample ed-cs-sample';
      sample.appendChild(document.createTextNode('Base text '));
      const styled = document.createElement('span');
      styled.className = 'mk-cs-' + c.id;
      styled.textContent = 'styled Ag';
      sample.appendChild(styled);

      const spec = document.createElement('div');
      spec.className = 'ed-typo-spec';
      spec.textContent = csSpecLine(c);
      ...
      edit.appendChild(csEmField('Size (em)', c.sizeEm, function (v) {
        if (v == null) delete c.sizeEm; else c.sizeEm = v;
        afterFieldEdit();
      }));

      edit.appendChild(selectField('Weight',
        (c.weight === 'bolder' || c.weight === 'lighter') ? c.weight : '',
        CS_WEIGHTS, function (v) {
          if (v === 'bolder' || v === 'lighter') c.weight = v; else delete c.weight;
          afterFieldEdit();
        }));

      edit.appendChild(selectField('Italic',
        (c.italic === true ? 'italic' : (c.italic === false ? 'normal' : '')),
        CS_ITALICS, function (v) {
          if (v === 'italic') c.italic = true;
          else if (v === 'normal') c.italic = false;
          else delete c.italic;
          afterFieldEdit();
        }));

      edit.appendChild(csEmField('Letter spacing (em)', c.letterSpacingEm, function (v) {
        if (v == null) delete c.letterSpacingEm; else c.letterSpacingEm = v;
        afterFieldEdit();
      }));
      ...
      setExpanded(!!expandedCsIds[c.id]);
      toggle.addEventListener('click', function () {
        setExpanded(edit.style.display === 'none');
      });
      ...
      nameInp.addEventListener('input', function () {
        c.name = nameInp.value;
        if (newCsIds[c.id]) {
          const newId = uniqueCsId(slugifyCsId(c.name), c.id);
          if (newId !== c.id) {
            const wasExpanded = !!expandedCsIds[c.id];
            delete newCsIds[c.id];
            delete expandedCsIds[c.id];
            c.id = newId;
            newCsIds[newId] = true;
            if (wasExpanded) expandedCsIds[newId] = true;
            li.setAttribute('data-cs-id', newId);
            idTag.textContent = 'cs-' + newId;
            styled.className = 'mk-cs-' + newId;
            rebuildCharstyleClientCss();
          }
        }
        markCharStylesDirty();
  ...
  async function saveCharStyles(btn) {
    if (btn) { btn.disabled = true; btn.textContent = 'Saving…'; }
    try {
      const res = await fetch('/dev/draw/charstyles', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ styles: state.charStyles })
      });
      const j = await res.json();
      if (!res.ok || !j.ok) throw new Error(j && j.error ? j.error : ('HTTP ' + res.status));
      if (Array.isArray(j.styles)) {
        state.charStyles = j.styles;   // adopt the server-normalised set
      }
      Object.keys(newCsIds).forEach(function (k) { delete newCsIds[k]; });
      rebuildCharstyleClientCss();
      renderCharStyleList();
      clearCharStylesDirty();
      if (btn) { btn.disabled = false; btn.textContent = 'Saved.'; }
      setTimeout(function () {
        const b = document.getElementById('save-charstyles-btn');
        if (b && !charStylesDirty) b.textContent = 'Save';
      }, 1800);
    } catch (err) {
      console.error('[dev-draw] char-styles save failed:', err);
      if (btn) { btn.disabled = false; btn.textContent = 'Failed'; }
      alert('Character-styles save failed: ' + err.message);
      setTimeout(function () {
        const b = document.getElementById('save-charstyles-btn');
        if (b) b.textContent = charStylesDirty ? 'Save •' : 'Save';
      }, 1800);
    }
  }
  ...
  // Character-style panel (TS4 Slice 3). Same one-time render as typography
  // — site-wide/shared, outside renderAll's per-edit churn. Live preview CSS
  // is (re)built up front so seed styles render WYSIWYG immediately.
  rebuildCharstyleClientCss();
  renderCharStyleList();
  const newCsBtn = document.getElementById('new-charstyle-btn');
  if (newCsBtn) {
    newCsBtn.addEventListener('click', addCharStyle);
  }
  const saveCsBtn = document.getElementById('save-charstyles-btn');
  if (saveCsBtn) {
    saveCsBtn.addEventListener('click', function () { saveCharStyles(saveCsBtn); });
  }
